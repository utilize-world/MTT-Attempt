...
class MultiAgentEnv(gym.Env):
    ...
    def render(self, mode='human'):
        ...
        if self.render_geoms is None:
            ...
            # 这里world.entities包括所有的实体，在core函数中为只读
            for entity in self.world.entities:
                geom = rendering.make_circle(entity.size)  # 调整为10

                xform = rendering.Transform()

                if 'agent' in entity.name:
                    geom.set_color(*entity.color, alpha=0.5)
                    geom_obs = rendering.make_circle(entity.size + 90, filled=False)  # add
                    x_obs = rendering.Transform()  # add
                else:
                    geom.set_color(*entity.color)
                geom.add_attr(xform)  # geom自身有属性self.attr,这里是将坐标形式等参数一起加入到attr中
                self.render_geoms.append(geom)  # 这个得到的就是有着各种参数的agent
                self.render_geoms.append(geom_obs)  # add

                self.render_geoms_xform.append(xform)  # 这个是顺序对应了geom的相关信息
                self.render_geoms_obs_xform.append(x_obs)  # add
        ...
        for i in range(len(self.viewers)):
            ...
            self.viewers[i].set_bounds(pos[0], pos[0] + cam_range, pos[1], pos[1] + cam_range)  # 重写至边界为0,0,800,800
            # update geometry positions
            for e, entity in enumerate(self.world.entities):
                self.render_geoms_xform[e].set_translation(*entity.state.p_pos)
                if entity in self.world.agents:
                    self.render_geoms_obs_xform[e].set_translation(*entity.state.p_pos)  # 显示观察环
