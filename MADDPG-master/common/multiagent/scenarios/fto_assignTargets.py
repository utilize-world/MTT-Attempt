...
class RewardNormalizer:
    def __init__(self, initial_mean=0.0, initial_std=1.0, alpha=0.01):
        # 初始化均值和标准差的移动平均
        self.mean = initial_mean
        self.std = initial_std
        self.alpha = alpha  # 移动平均更新率

    def update(self, value):
        # 更新均值和标准差
        self.mean = (1 - self.alpha) * self.mean + self.alpha * value
        self.std = (1 - self.alpha) * self.std + self.alpha * (value - self.mean) ** 2

    def normalize(self, value):
        # 返回标准化后的值
        return (value - self.mean) / (np.sqrt(self.std) + 1e-8)
# MLGA2C中的环境，为了简化，将原本的5v5变成3v3

class RewardNormalizers:
    def __init__(self):
        self.dis_reward_normalizer = RewardNormalizer()
        self.collision_reward_normalizer = RewardNormalizer()
        self.time_reward_normalizer = RewardNormalizer()
        self.shapley_reward_normalizer = RewardNormalizer()
        self.bound_reward_normalizer = RewardNormalizer()
        self.se_reward_normalizer = RewardNormalizer()
    def update(self, dis_reward, shapley_reward,collision_reward,time_reward,bound_reward,SE_reward):
        self.dis_reward_normalizer.update(dis_reward)
        self.shapley_reward_normalizer.update(shapley_reward)
        self.collision_reward_normalizer.update(collision_reward)
        self.time_reward_normalizer.update(time_reward)
        self.bound_reward_normalizer.update(bound_reward)
        self.se_reward_normalizer.update(SE_reward)
...
def calculate_relative_position_UAVs_returnWithArray(agent, other, world, fortarget=False):
    relative_position_map = []
    # 假设agent j的位置为pj，agent i的位置为pi， 则相对位置为|pj-pi|
    pi = agent.state.p_pos
    if not fortarget:
...
class Scenario(BaseScenario):
    def __init__(self, evaluate=False, safe_initialization=True):
        ...
        self.centers = [
        (2/6, 1.4),    # 区域1: (0.333, 1.4)
        (1.0, 1.4),     # 区域2: (1.0, 1.4)
        (10/6, 1.4),    # 区域3: (1.666, 1.4)
        (0.5, 0.4),     # 区域4: (0.5, 0.4)
        (1.5, 0.4)      # 区域5: (1.5, 0.4)
        ]
        self.sigma = 0.05 / 3 # 噪声参数，标准差
        # self.train = not evaluate # POMG TODO:
        self.train = True
        # self.train = evaluate
        np.random.seed(0)
    ...
    def make_world(self):
        ...
        world = World()
        world.comm_range = 0.4  # 原文中是获取最近的两个agent的数据，以及最近两个target的信息
        world.bound = 2  # 2mx2m

        # set any world properties first
        # world.dim_c = 2     # 通信维度在这里也能定义，之前在core中已经定义过为3*agent的个数，这里将其注释掉
        num_uav = 5  # 就是UAVs个数
        num_target = 5  # 这个是target个数
    ...
    def agent_reward(self, agent, world, agent_index):
        ...
        w1, w2, w3, w4, w5, wse = 1, 0, 0, 0, 1, 0  # w3 -> collision weight w 3
        ...
        # 对每个UAV计算其自身的reward
        for dis in current_uav_target_dismap:
            if dis <= agent.obs_range:
                dis_reward += 1 + (agent.obs_range - min_dis) / agent.obs_range

        #    ----global dis reward----

        for ele in dis_map:
            for dis in ele:
                #dis_reward +=  (agent.obs_range - dis) / agent.obs_range
                global_dis_reward += -dis
        dis_reward = 1 * dis_reward + 0 * global_dis_reward/ (len(self.get_agent(world)) * len(self.get_target(world)))

        agent_relative_pos = calculate_relative_position_UAVs_returnWithArray(agent, world.agents, world)
        #  TODO： 感知奖励
        # for rel_pos in agent_relative_pos:
        #     dis = np.linalg.norm(rel_pos)
        #     if dis > 0:
        #         sens_reward += dis - agent.obs_range * 1.5
        #     else:
        #         sens_reward -= 0.1
        ...
        # shapleyReward = calculate_shapley_value(n, lambda S: v(S, dis_map, dis_UAV_in_comm_index, 100))[sha_index]
        ...
        if self.isRewardNorm :
            self.rewardNorm.update(dis_reward,
                                   shapleyReward,
                                   collision_reward,
                                   time_reward,
                                   bound_reward,
            ...
            dis_reward = self.rewardNorm.dis_reward_normalizer.normalize(dis_reward)
            shapleyReward = self.rewardNorm.shapley_reward_normalizer.normalize(shapleyReward)
            collision_reward = self.rewardNorm.collision_reward_normalizer.normalize(collision_reward)
            time_reward = self.rewardNorm.time_reward_normalizer.normalize(time_reward)
            bound_reward = self.rewardNorm.bound_reward_normalizer.normalize(bound_reward)
            SE_reward = self.rewardNorm.se_reward_normalizer.normalize(SE_reward)
    ...
    def observation(self, agent, world):
        ...
        row_ind, target_ind = linear_sum_assignment(dis_map)
        ...
        #### TODO:check
        if not self.train:
            x_noise = np.random.normal(scale=self.sigma)
            y_noise = np.random.normal(scale=self.sigma)
            center = self.centers[agent_index]
            new_x = np.clip(center[0] + x_noise, 0, 2)
            new_y = np.clip(center[1] + y_noise, 0, 2)
            noise_pos = [new_x, new_y]
            target_relative_pos = cal_relative_pos(agent.state.p_pos, noise_pos)
        ####
        else:
            x_noise = np.random.normal(scale=self.sigma)
            y_noise = np.random.normal(scale=self.sigma)
            assigned_target_pos = world.targets_u[target_ind[agent_index]].state.p_pos
            assigned_target_pos = [assigned_target_pos[0] + x_noise, assigned_target_pos[1] + y_noise]
            target_relative_pos = cal_relative_pos(agent.state.p_pos, assigned_target_pos)
        #target_relative_pos = calculate_relative_position_of_other_agents(agent, world.targets_u, world, fortarget=True)
        mindis = min(dis_map[agent_index])
        for target_i, dis in enumerate(dis_map[agent_index]):

            if dis <= agent.obs_range:
                obs_pos.append(world.targets_u[target_i].state.p_pos)
                # 最近的分给前面
                if mindis == dis and not self.train:
                    target_relative_pos = cal_relative_pos(agent.state.p_pos,world.targets_u[target_i].state.p_pos)
            # ####
            else:
                obs_pos.append([0, 0])
        obs_pos = np.array(obs_pos).flatten()

        agent_relative_pos = calculate_relative_position_of_other_agents(agent, world.agents, world)
        # obs_cat = np.hstack((agent_p, velx, vely, target_relative_pos, agent_relative_pos)) # 2 + 1 + 1 + 2 + 2*(u-1)

        obs_cat = np.hstack((agent_p, velx, vely, target_relative_pos, obs_pos, agent_relative_pos))
        # obs_cat = np.hstack((agent_p, velx, vely, obs_pos, agent_relative_pos))
    ...
    def constraints(self, agent, world, max_cons=2):
        # Constraint Type 1: Collisions with other robots
        # max_cons代表每个agent的最大约束，最好与action——dim保持一致，这样能够保证correct时保持维度一致
        ...
        for i, other in enumerate(other_agents):
            ...
            # L2 norm default
        sort_cons = np.sort(collision_signals)
        # if np.empty(sort_cons):
        #     return collision_signals
        return sort_cons[:max_cons]
