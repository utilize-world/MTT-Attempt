...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.safe_range = 0.25  # 安全区域也为0.3m，这个范围内就不安全，会受到碰撞惩罚
    ...
    def agent_reward(self, agent, world, agent_index):
        ...
        if min_dis <= agent.obs_range:
            time_reward = 1 + (agent.obs_range - min_dis) / agent.obs_range
        ...
        for ele in dis_map:
            for dis in ele:
                ...
                dis_reward += -dis
        dis_reward = dis_reward / (len(self.get_agent(world)) * len(self.get_target(world)))
