...
class MLGA2C:
    ...
    def save_model(self, train_step, agent_id):
        ...
        model_path = os.path.join(model_path, 'MLGA2C')
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        model_path = os.path.join(model_path, 'agent_%d' % agent_id)
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        torch.save(self.policy.state_dict(), model_path + '/' + num + 'MLGA2C_actor_params.pkl')
        torch.save(self.qf1.state_dict(), model_path + '/' + num + 'MLGA2C_critic_q1_params.pkl')
        torch.save(self.qf2.state_dict(), model_path + '/' + num + 'MLGA2C_critic_q2_params.pkl')
    ...
    def train(self, transitions, agent_id):
        ...
        o_self = transitions['o_%d' % agent_id] # 这里的观察应该为 2 + 2 + 2n + 2m ,n和m分别代表目标数和通信范围(所有agent的数目)
        u_self = transitions['u_%d' % agent_id]
        o_next_self = transitions['o_next_%d' % agent_id]
        ...
        with torch.no_grad():
            ...
            for i in range(self.args.n_agents):
                if i == agent_id:
                    u_self_next, next_state_log_i, _, _ = self.policy.get_actions(o_next_self)
                    u_self_next_reshape = u_self_next.reshape(batch_size, 1, div_dim)
                    next_state_log.append(next_state_log_i)
                else:
                    u_next_i, next_state_log_i, _, _ = self.policy.get_actions(o_next[j])
                    j = j + 1
                    # 256*2, 256*1
            ...
            q1_next_target = self.qf1_t(o_next_self_reshape, o_next_other_reshape, u_self_next_reshape, u_other_next_reshape)
            q2_next_target = self.qf2_t(o_next_self_reshape, o_next_other_reshape, u_self_next_reshape, u_other_next_reshape)
        ...
        # # 传入的是state_own, state_others, action_own, action_others，且格式为256, n, 2
        qf1_a_values = self.qf1(o_self_reshape, o_other_reshape, u_self_reshape, u_other_reshape).view(-1)
        qf2_a_values = self.qf2(o_self_reshape, o_other_reshape, u_self_reshape, u_other_reshape).view(-1)
        ...
        if self.train_step > 0:
            ...
            # 如果确认更新
            u_pi_other, u_pi_other_reshape = [], []
            state_log_pi = []
            for i in range(self.args.n_agents):
                j = 0
                if i == agent_id:
                    u_pi_i, state_log_pi_i, mean, logstd = self.policy.get_actions(o_self)
                    u_pi = u_pi_i
                    u_pi_reshape = u_pi.reshape(batch_size, 1, div_dim)
                    state_log_pi.append(state_log_pi_i)
                else:
                    u_pi_i, state_log_pi_i, mean, logstd = self.policy.get_actions(o[j])
                    u_pi_other.append(u_pi_i)
                    u_pi_other_reshape.append(u_pi_i.reshape(batch_size, 1, div_dim))
                    state_log_pi.append(state_log_pi_i)
                    j = j + 1
            ...
            # 传入的是state_own, state_others, action_own, action_others，且格式为256, n, 2
            qf1_pi = self.qf1(o_self_reshape, o_other_reshape, u_pi_reshape, u_pi_other_reshape)
            qf2_pi = self.qf2(o_self_reshape, o_other_reshape, u_pi_reshape, u_pi_other_reshape)
            ...
            with torch.no_grad():
                ...
                for i in range(self.args.n_agents - 1):
                    ...
                    state_log_pi.append(state_log_pi_i)
                _, state_log_pi_i, _, _ = self.policy.get_actions(o_self)
                state_log_pi.append(state_log_pi_i)
                log_pi = einops.reduce(
                    state_log_pi, "c b a -> b a", "sum"
                )
